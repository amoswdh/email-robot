...
class EmailAccount extends Model
{
    ...
    public function createTicket(Mailbox $mailbox, $mailId)
    {
        ...
        $email_cs_admin_id_from_tel = isset($tmp_order_sn[2]) && !empty($tmp_order_sn[2]) ? $tmp_order_sn[2] : "";
        ...
        if (property_exists($mail, "references") && self::isOldEmail($mail->references)) {
            ...
            if (!is_object($existEmail)) {
                //修复缺失的主信息
                $this->saveMail($mailbox, array_merge($data, [
                    "message_id" => $data['p_message_id'],
                    "p_message_id" => 0,
                    "date" => time() - 86400 * 2,
                    "is_virtual" => 1 //修复标记
                ]), array_merge($data_content, [
                    "message_id" => $data['p_message_id'],
                    "text_plain" => "",
                    "text_html" => ""
                ]), $order_sn_ticket);
                ...
                //保存主数据
                $this->saveMail($mailbox, array_merge($data, ['references_str' => $mail->references, 'in_reply_to' => $mail->in_reply_to]), $data_content, $order_sn_ticket, false);
            } else {
                $ticket = $this->getTicket($data);
                if (is_object($ticket)) {
                    $this->saveMail($mailbox, array_merge($data, ['references_str' => $mail->references, 'in_reply_to' => $mail->in_reply_to]), $data_content, $order_sn_ticket, false);
                    //打开Ticket
                    EmailTicketNew::where("ticket_id", $ticket->ticket_id)->update(
                        ['sys_flag' => 'opened', 'update_time' => time(), 'last_reply_time' => time()]
                    );
                } else {
     ...
     * @return bool
     */
    public function saveMail(Mailbox $mailbox, array $inboxData, array $inboxContentData, $order_sn_ticket, $add_ticket = true)
    {
        //声明变量基础值
        $emailInbox = null;
        ...
        if (is_object($emailInbox)) {
            ...
            if ($add_ticket) {
                ...
                $ticket = $this->addTicket($emailInbox, $p_message_id, $order_sn_ticket);
            }
        }
     ...
     * @return static
     */
    public function addTicket(EmailInbox $emailInbox, $p_message_id, $order_sn_ticket, $email_cs_admin_id = 11)
    {
    ...
    public function assignAgent()
    {
        ...
        if (is_null($email_cs_admin_id)) {
            $ticket_data['email_cs_admin_id'] = M('email_distribution')->where(array('email' => $emailInbox->reply_to_email))->find()['email_cs_admin_id'];
        }
